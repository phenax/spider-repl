...
export const browsers = (): Record<Browser, ProtocolAdapter> => {
  ...
  return {
    ...
    chrome: new DevtoolsProtocolAdapter({
      command: 'chrome',
      args: args => [...args, `--user-data-dir=${browserConfigDir('chrome')}`],
      beforeStart: () => setupConfigDir('chrome'),
    }),
    chromium: new DevtoolsProtocolAdapter({
      command: 'chromium',
      args: args => [...args, `--user-data-dir=${browserConfigDir('chromium')}`],
      beforeStart: () => setupConfigDir('chromium'),
    }),
    firefox: new WebDriverBiDiProtocolAdapter({
      command: 'firefox',
      args: args => [...args, '--new-instance', '-P', 'spider-repl', '--profile', browserConfigDir('firefox')],
